...
rem Update here: https://github.com/xiao-shen/sync_robocopy
rem 
...
if not exist %fol% (
  ...
  ECHO   \Subfolder1\
  ECHO   \Subfolder2\
  ECHO   ; add robocopy arguments:
  ECHO   \Subfolder3\^|^<arguments^>
  ECHO   ; copy particular file or files:
  ECHO   \Subfolder3\^|filename
  ECHO   ; exclude particular file:
  ECHO   \Subfolder3\^|/xf sync_robo_hist.log
  ECHO   ; exclude particular directories:
  ECHO   \Subfolder3\^|/xd FolderToExclude
  ECHO   ; exclude files larger than ^<N^> bytes:
  ECHO   \Subfolder3\^|/max:^<N^>
...
for /F "eol=; tokens=1* delims=|" %%i in (%fol%) do (
  ...
  IF not "!subFolder:~0,1!"=="\" (
    ...
    if "!fromSrc:~-1!"=="\" (
      ...
      ECHO robocopy "!fromSrc:~0,-1!" "!toDest:~0,-1!" %%j /e /r:1 /w:2 /log+:%log% /tee %opt%
      robocopy "!fromSrc:~0,-1!" "!toDest:~0,-1!" %%j /e /r:1 /w:2 /log+:%log% /tee %opt%
    ) else (
      ECHO robocopy "!fromSrc!" "!toDest!" %%j /e /r:1 /w:2 /log+:%log% /tee %opt%
      robocopy "!fromSrc!" "!toDest!" %%j /e /r:1 /w:2 /log+:%log% /tee %opt%
    )
  )
)
:HIST
